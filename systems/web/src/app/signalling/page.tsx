  ...
  return (
    <>
      <PageHeader
        ...
        lead="In a market with no shared prices, shares or benchmarks, actors articulate their position through observable signals. A signal informs to the extent it is costly and hard to fake. The core question: do actors substitute low-cost signals for costly capability evidence?"
      />

      <div className="grid cols-3">
        <Stat
          label="Ecosystem low-cost signal share"
        ...
        <Stat label="Low-cost signals" value={sig.cost_mix.low || 0} help="Roadmaps, positioning statements, branding." />
      </div>
      ...
      <div className="grid cols-2" style={{ marginTop: "1rem" }}>
        <Card>
          ...
          <p className="small muted" style={{ marginTop: 0 }}>
            Costly signals (Rieger et al. 2025; Suchman 1995) separate genuine capability from low-cost signalling.
          </p>
          <Donut data={costData} />
        </Card>
        <Card>
      ...
      <Card style={{ marginTop: "1rem" }}>
        <h3 style={{ marginTop: 0 }}>Do low-cost signals track costly ones?</h3>
        <p className="small muted" style={{ marginTop: 0 }}>
          Each point is an actor. X = share of their signals that are low-cost; Y = Cost-Weighted
          Signal Score. An actor high-right is loud but lightly-backed; an actor high-left is
          backing its position with costly evidence.
        </p>
        {scatter.length ? (
          <Scatter2D data={scatter} xKey="cheap_talk" yKey="credibility" xLabel="Low-Cost Signal Share (%)" yLabel="Cost-Weighted Signal Score" />
        ) : (
        ...
        <table>
          <thead>
            <tr>
              ...
              <th className="num">Cost-weighted</th>
              <th className="num">Low-cost %</th>
              <th className="num">High-cost</th>
              <th className="num">Cap.–Leg.</th>
            </tr>
          </thead>
          <tbody>
      ...
      <p className="small faint" style={{ marginTop: "1rem" }}>
        Cost-Weighted Signal Score = Σ (dimension weight × confidence × cost multiplier). Cost
        multipliers: high 1.0, medium 0.7, low 0.4. See the <a href="/methodology">methodology</a> for
        the full model and per-dimension grounding.
      </p>
    </>
  );
}
