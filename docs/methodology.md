...
- **Multi-agent debate Critic** (Du et al. 2023) on System A. The consensus Critic above is the *independent-samples* pattern (Wang et al. 2023); the *inter-pass-debate* pattern, where each pass sees the others' arguments and revises, is a deliberate next step rather than a v1 feature. Worth adding only if literature review justifies it and a measurable gap appears.
- **Semantic deduplication in the Critic** — the embeddings are now persisted, but the Critic still dedups only by exact (actor_slug, source_url, content_hash). Vector-cosine dedup is the natural next step once the embedding corpus is populated.
